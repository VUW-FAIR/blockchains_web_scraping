...
class btccom_forum_scraper(scrapy.Spider):
    name = "btccom"
    forum_folder_name = "forumbitcoincom"
    start_urls = ['https://forum.bitcoin.com/']
    ...
    def parse_page(self, response):
        ...
        usernames = response.selector.xpath('//a[contains(@class, "username")]//@href').extract()
        ...
        csv_file = open(".\\" + self.forum_folder_name + "\\" + title[0].replace("/", "").replace("\\", "").replace("|", "").replace(
"*", "").replace("?", "").replace('"', "").replace("<", "").replace(">", "").replace(":", "").replace("-", "_").replace(".", "") + ".csv", "a", encoding="utf-8")  # or append?
        ...
        if (len(usernames) == len(post_times) == len(post_text)):
            ...
            for i in range(0, len(usernames)):
                ...
                writer.writerow([(title[0].strip()), (usernames[i].strip()), post_times[i],
                                 (post_text[i].strip())])
        else:
